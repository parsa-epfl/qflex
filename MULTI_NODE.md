...
- **`savevm` over the monitor is master-only under multi-node.** PDES `savevm` triggers a `DRAIN_START` / `DRAIN_END` coordination across every node and persists per-node CPU+memory state as part of the distributed snapshot. Node 0 sends the command; non-master nodes use a *different* `interaction_script` whose job is to keep their QEMU alive (so they can participate in the drain) until the master finishes — typically by polling a sentinel file the master writes after `savevm` returns. Wiring the same `interaction_script` to every leaf when a snapshot is involved is a bug. See [conf/DC/dc-multi-savevm-create.yaml](conf/DC/dc-multi-savevm-create.yaml) for the canonical split (`boot_create_and_savevm_master.exp` for node 0, `boot_create_and_wait.exp` for the rest) and the `boot-load-interactive` skill for more.
