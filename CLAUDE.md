...
- **VERY IMPORTANT — minimal change, or ask first.** No matter what you're implementing, do it with as few changes as possible. If the task genuinely requires a big change (broad refactor, cross-cutting rewrite, new abstraction layer, anything touching many files or core machinery), STOP **at the start** and surface the proposed scope to the user before writing any code. Don't quietly expand scope mid-edit; don't "while I'm here" cleanups. Confirm the big move, then do it.
- **VERY IMPORTANT — NEVER run git.** No `git checkout`, no `git commit`, no `git push`, no `git reset`, no `git stash`, no `git rebase`, no `git restore`, no `git merge` — NOTHING that touches git state. The user manages git themselves. Even read-only-looking flows like `git checkout -- <file>` to "revert just my changes" are forbidden because they silently destroy any uncommitted user work in the same file. To undo your own edits, use the Edit tool to put the original text back; if the original is gone, ask the user. The ONLY git commands that are OK are pure reads with no side effects (`git status`, `git diff`, `git log`, `git show`, `git fsck --lost-found`) — and even those, only when you actually need them to diagnose something the user asked about.
...
- **Real-run test settings live in YAML, not in python.** Every knob a real-run test depends on (experiment_name, interaction_script, loadvm_name, use_gdb, …) belongs in a per-test YAML under [tests/realrun/](tests/realrun/), not as a CLI override in the test code. The python test should be reduced to `_exec_in_container("./qflex <phase> -c tests/realrun/<x>.yaml")` plus the assertions on the captured output. Test-specific expect scripts (`*.exp`) live next to their YAMLs in [tests/realrun/](tests/realrun/) so the fixture file set is self-contained. The YAMLs `extends: <base>` (the full-fidelity bases `base.yaml` / `single.yaml` / `multi.yaml`, now colocated under [tests/realrun/](tests/realrun/)) to inherit defaults — change the base, every test follows. Knobs that aren't already on `ExperimentContext` (`use_gdb` was the recent one) need to be added there so YAML can set them.
...
- **Don't wait endlessly on real-run tests.** Real-run tests pass in 1–2 minutes; if a multi-node leaf gets stuck in PDES sync the test can sit through the full 30-min `_exec_in_container` timeout. Don't sit there waiting for the runtime's "command completed" notification — actively check whether progress is happening. The shape that works (in this order):
  1. Run the test **foreground** (NOT `run_in_background: true`, NOT `Monitor` — those write to `/tmp` and brick the boot disk) at the longest reasonable Bash `timeout`. Output streams back as the tool result.
  2. While waiting, peek at the experiment folder's existing logs with `Read`: `<mounting>/experiments/<sub>/Load.log`, `Load.err`, `expect_log.txt`, `ls_after_*.txt` — all written by the executor / expect script in real time, no extra redirects needed.
  3. Cross-check `ps -ef | grep qemu-system | grep -v grep` and `ls /dev/shm/pdes*`. If the captures are already written but qemu is still chewing CPU and the expect log shows no new bytes, it's stuck — diagnose now, don't wait another 25 minutes. Most likely cause: peer-kill didn't fire because one leaf exited too early (see the "make sure they all die together" rule above).
  4. Never bake any of this into the python test code; the test stays one-line-per-step minimal. The polling lives on the Claude side via `Bash` + `Read`.
...
- [./dep](dep) — host-side. Builds and starts the QFlex Docker dev container.
  ...
  - **Before running `./dep build-docker`, ASK the user: debug or release? worm or no-worm?** Never pass `--push`.
...
A pytest suite lives under [tests/](tests/), one file per pipeline component (`test_boot.py`, `test_load.py`, …, `test_run_partition.py`). Default `make test` ([Makefile](Makefile)) runs only the dry-run-based tests — fast and hermetic. A separate set of real-run files ([tests/test_dev_container_smoke.py](tests/test_dev_container_smoke.py), [tests/test_chained_pipeline.py](tests/test_chained_pipeline.py), [tests/test_dev_image_contents.py](tests/test_dev_image_contents.py)) drive `./dep start-docker --background` + `./dep exec` to actually run qflex inside the dev container; gated behind `QFLEX_REAL_RUN_TESTS=1` so default `make test` skips them. To run a single real-mode test or file, use `make test-real-one TEST=tests/test_dev_image_contents.py[::test_name]`. See the `testing` and `dep` / `run-in-dev-container` skills for the conventions used there (capture stdout, parse `[py-wait]` / `[py-touch]` / `[bash]` / `[tmux]` markers, master-first invariants, and the start-bg → exec → stop-bg session pattern).
...
**Iterating on PDES / QEMU source: use `make test-iterate`, NOT `./dep build-docker`.** The `compile_in_container` test ([tests/container_compiler.py](tests/container_compiler.py), opt-in via `QFLEX_ITERATE=1`) recompiles `qemu/` and `parallel-qemu/` inside the **same `qflex_test` container the real-run tests use** (TEST_CONTAINER_NAME from [tests/conftest.py](tests/conftest.py)). It reuses the prebuilt `-saved/` trees baked into the image, so only changed `.c`/`.h` files recompile (seconds-to-minutes vs. the 30+ minute full image rebuild). The container is **left running** after the recompile; the `dev_container` fixture detects a pre-existing `qflex_test` and reuses it (no stop+start), so the recompiled binaries actually reach the real-run tests. To clean up the container manually after iteration: `./dep stop-docker --container-name qflex_test`. Claude must NEVER invoke `test-iterate` autonomously — only when the user explicitly asks.
...
- **WS `loaded` multi-node snapshot carries undrained in-flight PDES messages.** A `generate-test-communication` diag run on WS multi-node showed node-1 `[CKPT-INFLIGHT] loaded: restored 27 in-flight messages` (node-0: 0) on loadvm — i.e. the load-phase `savevm loaded` drained to a non-zero in-flight count. We believe the drain should land at 0 in-flight; 27 restored means the `DRAIN_START`/`DRAIN_END` round at the `loaded` checkpoint either didn't complete or the workload kept injecting after drain. Investigate the load-time drain path in `qemu-pdes/net/pdes-checkpoint.c` (and whether WS's cross-node search traffic outruns the drain). Symptom-only so far; revisit.
- ~~**`END_OF_EMULATION` reception races against Flexus's final `all.measurement.end.log` flush.**~~ FIXED in `qemu-pdes/net/pdes-engine.c`. `can_stop` on both master and follower now returns `permitted_to_exit && pair_has_finished`, and END is emitted once `permitted_to_exit` is set (decoupled from `notify_neighbours_of_end`). Both nodes therefore keep ticking through the natural WWT barrier until each has both granted/received the handshake AND received the peer's END — at which point both pass `can_stop` and terminate together at the same barrier (each writing `end.log`, then exiting). Test asserts EVERY `all.measurement.end.log` is present (`MAX_MISSING_END_LOG_PER_TEST = 0` in `tests/test_chained_pipeline.py`); test_07–test_10 pass at strict 0-budget.
...
- The `dc-multi-savevm-create.yaml` fixture (a `tests/realrun/` savevm-create example) is the canonical split: node 0 → `boot_create_and_savevm_master.exp`, node 1 → `boot_create_and_wait.exp`. Never wire the same `interaction_script` to both leaves when a snapshot is involved.

**Multi-node `boot` runs at high latency / no sync; simulation phases run at low latency / with sync.** Concretely: per-leaf `latencies_ns_list: [1000000]` (1 ms) + `syncs_list: ["false"]` for boot; per-leaf `latencies_ns_list: [100000]` (100 µs, the default in [tests/realrun/multi.yaml](tests/realrun/multi.yaml)) + `syncs_list: ["true"]` for `load` / `fw` / `init-warm` / `run-partition`. Without `MSG_TYPE_SYNC` exchanges the two nodes' virtual times can drift; bumping the wire latency to 1 ms gives senders enough headroom that their timestamps stay ahead of the receiver's now-virtual-time. Simulation phases need `syncs_list: ["true"]` regardless of latency — `SimulationCommand._assert_syncs_true` in [commands/executer.py](commands/executer.py) enforces this because per-sample alignment depends on it. `Boot` does not extend `SimulationCommand`, which is what makes the no-sync override safe there.
