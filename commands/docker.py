...
class DockerBuild(Executor):
    ...
    def cmd(self) -> str:
        ...
        local_qflex_build_name = f"{self.docker_base_image_name}-build:{self.version}"
        ghcr_qflex_build_name = f"ghcr.io/parsa-epfl/qflex:{self.docker_base_image_name}-build-{self.version}"
        ...
        if not self.worm_only:
            ...
            base_image_push_cmd = [
                f"docker push {ghcr_qflex_build_name}",
        ...
        worm_image_cmd = [
            f"""
            docker buildx build --load -t {local_worm_name} --build-arg BASE_IMAGE={local_qflex_name} -f Dockerfile.WormCacheQFlex .
            """,
        ...
        qpoints_image_cmd = [
            f"""
            docker buildx build --load -t {local_qpoints_name} --build-arg BASE_IMAGE={local_qflex_name} -f Dockerfile.QPoints .
            """,
